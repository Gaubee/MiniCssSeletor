		...
		default:
			...
			switch(pseudo_name){
				case "has":
					...
					//获取最顶层
					var top_selector = selector;
					while(top_selector.parent){
						top_selector = top_selector.parent;
					}
					//如果第一个字符不是关系选择符，那么需要加上" "
					if (!_parse_handle_hash[pseudo_arg.charAt(0)]) {
						var ttop_s = new Seletor(" ",[[]]);
						top_selector.parent = ttop_s;
						top_selector = ttop_s;
					}
					top_selector.parent = parent;

					for(var i = 0,ele;ele = eles[i];i+=1){
						parent.content = [ele];
						var filter_eles = selector._run();
						if (filter_eles.length) {//如果有对应的子元素
							content.push(ele);
						}
						selector._reset();
					}
				...
				case "nth-last-child":
					for(var i = 0,ele;ele = eles[i];i+=1){
						//减法时，参数格式自动转化
						ele = ele.children[ele.children.length-pseudo_arg];
						ele&&content.push(ele);
					}
					break;
			}
