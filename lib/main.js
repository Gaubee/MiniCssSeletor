...
function ele_by_class(ele, className) {
	return ele.getElementsByClassName(className);
};
function ele_by_tag(ele, tagName) {
	return ele.getElementsByTagName(tagName)
};
function has_attr(ele,attr_name) {
...
//暴露相关的配置从而支持拓展的插件
$.parse_handle_hash_no_relationship = _parse_handle_hash_no_relationship;
$.parse_handle_hash = _parse_handle_hash;
$.PARSER = PARSER;
$.handle = _code;
