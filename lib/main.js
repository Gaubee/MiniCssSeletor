...
function $(str) {
	...
	for(var i = 0,c,parse_info;c = str[i];/*i+=1*/){
		...
		switch (c){
			case "#":
				parse_info = PARSER.ID(str,i);
				i = parse_info.i;
				// console.log(i);
				result = result._link_child(parse_info.type,parse_info.args);
				break;
			case ".":
				parse_info = PARSER.CLASS(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			case " ":
				parse_info = PARSER.EMPTY(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			case ">":
				parse_info = PARSER.CHILD(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			case "+":
				parse_info = PARSER.ADJACENT(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			case "~":
				parse_info = PARSER.BROTHER(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			case "[":
				parse_info = PARSER.ATTR(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			case "*":
				parse_info = PARSER.ALL(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
				break;
			default:
				// console.log("未知字符，可能是标签：",c);
				parse_info = PARSER.TAG(str,i);
				i = parse_info.i;
				result = result._link_child(parse_info.type,parse_info.args)
		}
	}
